        ...
        if not self.ball_served:
            ...
            def hit_block_bottom():
                ...
                if ball[1] > 260:
                    ...
                    if block_hit - 5 < ball_hit < block_hit + 30:
                        return [ball_hit, 260], [velocity[0], -velocity[1]]
                    else:
                        return None
                else:
                    return None
            ...
            if speed[1] > 0:
                ...
                if hit_block_side() is not None:
                    ...
                    self.destination_1p = reflect(point, velocity)
                    if scene_info['platform_1P'][0] + 18 < self.destination_1p < scene_info['platform_1P'][0] + 22:
                        command = 'NONE'
                    elif scene_info['platform_1P'][0] + 18 > self.destination_1p:
                        command = 'MOVE_LEFT'
                    else:
                        command = 'MOVE_RIGHT'
                else:
                    self.destination_1p = reflect(ball, speed)
                    if scene_info['platform_1P'][0] + 18 <= self.destination_1p <= scene_info['platform_1P'][0] + 22:
                        command = 'NONE'
                    elif scene_info['platform_1P'][0] + 18 > self.destination_1p:
                        command = 'MOVE_LEFT'
